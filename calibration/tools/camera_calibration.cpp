...
int main(int argc, char* argv[])
{
    help();
    Settings s;
    const string inputSettingsFile = argc > 1 ? argv[1] : "settings/camera_settings.xml";
    ...
    ofstream badImageLog, goodImageLog;
    badImageLog.open("bad_image_log.txt");
    goodImageLog.open("good_image_log.txt");
	...
	// detect corners in image space
        ...
        if ( found)                // If done with success,
        {
              // improve the found corners' coordinate accuracy for chessboard
                ...
                string goodRawImage = s.getRawImage();
                goodImageLog << goodRawImage << std::endl;

        } else
        {
            string badRawImage = s.getRawImage();
            badImageLog << badRawImage << std::endl;
        }
...
static void save_reproj_errs(vector<float> reprojErrs)
{
    ofstream reprojection_log;
    reprojection_log.open("reprojection_log.txt");
    reprojection_log << "Reprojection errors:\n";

    for (int i=0; i<reprojErrs.size(); i++)
    {
        cout << "reprojection errors!\n";
        cout << reprojErrs[i] << "\n";
        reprojection_log << reprojErrs[i] << "\n";
    }
}
...
bool runCalibrationAndSave(Settings& s, Size imageSize, Mat&  cameraMatrix, Mat& distCoeffs,vector<vector<Point2f> > imagePoints, vector<float> reprojErrs )
{
